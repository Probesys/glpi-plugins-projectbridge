...
class PluginProjectbridgeTicket extends CommonDBTM
{
   ...
   public static function postShow(Ticket $ticket) {
      ...
      if (true) {
         ...
         if (empty($project_id)
            ...
            $html_parts[] = '<input type="submit" name="update" value="'.__('Make the connection').'" class="submit" style="margin-left: 10px" />' . "\n";
   ...
   public static function deleteProjectLinks($ticket_id) {
      ...
      if ($nb_links != 0) {
         ...
         Log::history($ticket_id, 'Ticket', [0, '', __('Link(s) with project task(s) deleted')], 0, Log::HISTORY_LOG_SIMPLE_MESSAGE);
      }
   ...
   public static function processMassiveActionsForOneItemtype(MassiveAction $ma, CommonDBTM $item, array $ids) {
      switch ($ma->getAction()) {
         ...
         case 'addProjectTaskLink':
            if ($item->getType() == 'Ticket'
                 && !empty($ma->POST['projectbridge_projecttask_id'])
             ) {
